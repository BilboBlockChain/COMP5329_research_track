...
#get batches
dataset, checkpoint = preprocess(tokenizer_type = RobertaTokenizer, checkpoint = 'roberta-base') #pull bert tokenised data and checkpoint from preprocess
...
# get model
from bert_model import RoBERTa_classifier

criterion = nn.CrossEntropyLoss()
device = torch.device("cuda")

model = RoBERTa_classifier(checkpoint, NUM_CLASS) # uses same checkpoint as bert tokenizer - dont worry about the warning, it is expected while we are extracting only the CLS token
model.to(device)
